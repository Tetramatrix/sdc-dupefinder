...
Plugin URI: git://
Description: A fully scalable WordPress product matcher.
Version: 1.0
Author: Chi Hoang
Author URI: https://github.com/Tetramatrix 
Date: 28.02.2022

Note: In your theme template put <?php echo do_shortcode('[super_speedy_product_matcher posts_per_page=6 title="Searched 24,144 products for a match. Possible dupes found..."]')?>. 
For example the rehub theme put it in single-default.php.
*/
...
function matcherCallback() {
  ...
  $query = "SELECT wp_posts.ID, wp_posts.post_title, GROUP_CONCAT(tt.term_id SEPARATOR ',') as termcount, count(tt.term_id) as tterms, ct.termid as category, c.termid as matchid, c.num as matches, Round(if(count(tt.term_id)>{$term_count},c.num*({$term_count}/count(tt.term_id))/{$term_count}*100,c.num*(count(tt.term_id)/{$term_count})/{$term_count}*100)) as percentage
						...
						GROUP BY ID ) AS c
						ON (wp_posts.ID = c.ID AND wp_posts.ID NOT IN ({$post_id}) )
						INNER JOIN (
						SELECT object_id as ID, GROUP_CONCAT(tt.term_id SEPARATOR ',') as termid, count(tt.term_id) as num
						FROM wp_term_relationships as tr FORCE INDEX (idx3)
						INNER JOIN wp_term_taxonomy tt FORCE INDEX (PRIMARY)
						...
						GROUP BY ID ) AS ct
						ON (wp_posts.ID = ct.ID AND FIND_IN_SET ('2775', ct.termid) = 0 )
